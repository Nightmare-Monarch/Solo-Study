...
function addSession(subjectCode, totalMinutes, isPP, isQuiz) {
  ...
  // 15h = 100 marks => 1 min = 0.111 marks
  ...
  if (todayIndex === -1) dailyProgress.push({ date: today, minutes: totalMinutes });
  else dailyProgress[todayIndex].minutes += totalMinutes;
...
function updateUI() {
  ...
  subjects.forEach((s, index) => {
    ...
    div.className = "subject scroll-appear";
    div.innerHTML = `
  ...
  summaryContainer.innerHTML = `
    <strong>Total Marks: ${totalMarks.toFixed(1)} / 900</strong>
